...
# ===== SUBJECTS =====
SUBJECTS = {
    "math": "📐 Математика", "physics": "⚡ Физика",
    "history": "🏛 История КЗ", "biology": "🧬 Биология",
    "chemistry": "🧪 Химия", "geography": "🌍 География",
    ...
    if answered:
        return InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("📚 Теория", callback_data="theory"),
             InlineKeyboardButton("🔄 Сменить предмет", callback_data="change_subj")]
        ])
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("А", callback_data="ans_0"),
         InlineKeyboardButton("Б", callback_data="ans_1"),
         InlineKeyboardButton("В", callback_data="ans_2"),
         InlineKeyboardButton("Г", callback_data="ans_3")],
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"Активен ещё: {days_left} дней\n\n"
        f"📚 8 предметов ЕНТ\n"
        f"🤖 AI объяснения каждого ответа\n"
...
async def cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if d.startswith("subj_"):
        ...
        await q.edit_message_text(
            f"✅ *{SUBJECTS[user['subject']]}*\n\nВопросов сегодня осталось: *{left}*",
            parse_mode="Markdown",
        ...
        if user["questions_today"] >= get_limit(user):
            await q.edit_message_text("📵 Лимит исчерпан. Оформи подписку!", reply_markup=pay_kb())
            return
        await q.edit_message_text("⏳ Генерирую вопрос...")
        try:
            question = generate_question(user["subject"], user["plan"])
            user["current_q"] = question
            ...
            text = (
                ...
                f"В) {question['opts'][2]}\n"
                f"Г) {question['opts'][3]}\n\n"
                f"📝 {user['questions_today']}/{get_limit(user)}"
            )
        ...
        if chosen == correct:
            ...
            result = f"❌ *Неверно.* Правильный: *{keys[correct]}) {question['opts'][correct]}*"
        save_user(user)
        ...
        if not user.get("current_q"):
            await q.answer("Сначала загрузи вопрос!", show_alert=True); return
        if user["plan"] == "free":
            await q.answer("💡 Подсказки доступны в тарифе Обычный и VIP", show_alert=True); return
        ...
        if not user.get("current_q"): return
        if user["plan"] == "free":
            await q.answer("📚 Теория доступна в тарифе Обычный и VIP", show_alert=True); return
        question = user["current_q"]
        ...
        await q.edit_message_text(
            ...
            f"📱 Номер: `{KASPI_PHONE}`\n"
            f"💰 Сумма: *{price} тенге*\n\n"
            f"После оплаты нажми ✅ Я оплатил и пришли скриншот",
...
async def stats_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"📝 Всего: {user['total_questions']}\n"
        f"🎯 Точность: {pct}%\n"
        f"🏆 Прогноз ЕНТ: {round(pct/100*140)}/140",
        parse_mode="Markdown"
    )
